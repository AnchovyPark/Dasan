"""search — 파일 내용을 정규식으로 검색(grep 유사). 상대경로/기본은 workspace 기준(읽기 제한 없음)."""
...
class SearchInput(BaseModel):
    ...
    path: str = Field(default=".", description="검색 시작 경로 (파일 또는 디렉터리, 기본: 작업 폴더)")
...
def make_search_tool(ws: Workspace) -> Tool:
    def _run(inp: SearchInput) -> str:
        root = ws.resolve(inp.path, must_be_inside=False)
        if not root.exists():
        ...
        for f in _iter_files(root):
            ...
            try:
                ...
                continue
            if b"\x00" in data[:1024]:  # 바이너리 추정 → 스킵
                continue
