...
dasan start --session <제목>   # 저장된 세션 이어가기
dasan ask "이 파일 요약해줘: ./notes.txt"   # 단발 질문 (스크립트용, 기본=최근 세션)
dasan ask --session bongsu "상태 봐줘"     # 지정 세션에 단발 질문
dasan serve --host 127.0.0.1 --port 8790 --reload  # Bongsu용 로컬 API 서버
dasan workspace [경로]         # 작업 폴더 보기/변경
dasan list                     # 세션 목록 (제목·생성일·메시지 수)
dasan update --branch main     # 최신 버전으로 업데이트 (pipx/개발 설치 자동 감지)
