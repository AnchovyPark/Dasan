...
```bash
python3 -m agent.main                    # 대화형 REPL (종료: exit)
python3 -m agent.main "질문..."          # 단발 질문
...
**설계**: 루프/도구/세션은 프로바이더를 모른다. 어댑터의 정규화 타입만 사용 → 어댑터 교체로 모델/프로바이더 전환. 도구는 등록과 노출을 분리하고, 입력은 Pydantic으로 검증.
